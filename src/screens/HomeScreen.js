...
export default class HomeScreen extends React.Component {
    ...
    render = () => (
        <SafeAreaView style={[styles.container, styles.centerAlign]}>
            <View
                style={styles.greetingMessageContainer}
            >
                <Text
                    style={[styles.greetingMessage, styles.greetingsExtraStyle]}
                >{this.state.greetingMessage}</Text>
                <Text
                    style={[styles.greetingMessage, styles.greetingsExtraNameStyle]}
                >{this.state.name}</Text>
            </View>
            <View style={{height: 50}} />
            {
                this.state.isLoading ?
                <ActivityIndicator size={'large'} color={'#0000FF'} style={{marginVertical: 30}} /> :
                <FlatList
                    data={this.state.todoFocused ? this.state.todos : this.state.posts}
                    extraData={this.state.todoFocused ? this.state.todos : this.state.posts}
                    ...
                    renderItem={({item, index}) => (
                        ...
                        >
                            <Text
                                style={[styles.listTitleTextStyle, {fontWeight: 'bold', fontSize: 17}]}
                            >{item.title}</Text>
                            {
                                item.body &&
                                <Text
            ...
            >
                <RBSheetContent 
                    ...
                    focusTodo={() => this.setState({todoFocused: true, postFocused: false, isLoading: true, todos: [], posts: []}, this.fetchList)} 
                    focusPosts={() => this.setState({todoFocused: false, postFocused: true, isLoading: true, todos: [], posts: []}, this.fetchList)}                     
                />
            </RBSheet>
            <RBSheetContent 
